...
// Admin helper: run the 2025-11-03_add_verticals.sql migration safely
// Usage: open /admin/run_migration_add_verticals.php in browser (admin-only env recommended)
...
$sqlFile = __DIR__ . '/migrations/2025-11-03_add_verticals.sql';
if (!is_file($sqlFile)) {
...
foreach ($clean as $idx => $stmt) {
    try {
        $pdo->exec($stmt);
        $executed++;
        echo "<span class='ok'>[OK]</span> Statement ".($idx+1)." executed.\n";
    } catch (Throwable $e) {
        $errors++;
        echo "<span class='err'>[ERR]</span> Statement ".($idx+1).": ".htmlspecialchars($e->getMessage(), ENT_QUOTES, 'UTF-8')."\n";
    }
}

echo "\n<hr/><strong>Done. Executed: {$executed}, Errors: {$errors}.</strong></pre></body></html>\n";
