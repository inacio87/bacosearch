<?php
/**
 * /api/streets.php - Endpoint para resultados de ruas
 *
 * RESPONSABILIDADES:
 * 1. Receber parâmetros de localização e filtros via POST/GET.
 * 2. Consultar a tabela `streets` com fallback (cidade > região > país > global).
 * 3. Retornar dados em JSON para consumo por results_streets.php.
 */
...
try {
    ...
    switch ($action) {
        case 'get_streets':
            ...
            $results = findStreetsWithFallback($location_data, $filters);
    ...
    error_log("Erro na API streets: " . $e->getMessage() . " | Stack: " . $e->getTraceAsString());
