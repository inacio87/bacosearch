<?php
/**
 * /api/api_register_businesses.php - Cria/Atualiza cadastro de Empresas (companies)
 * Política: SEMPRE status='pending' e is_active=0 até aprovação manual no admin.
 */
...
function table_columns(PDO $pdo, string $table): array {
  ...
  $st->execute([$table]); $cols=$st->fetchAll(PDO::FETCH_COLUMN) ?: []; return $cache[$table]=array_flip($cols);
}
function filter_for_table(PDO $pdo,string $t,array $src): array { $cols=table_columns($pdo,$t); $out=[]; foreach($src as $k=>$v){ if(isset($cols[$k])) $out[$k]=$v; } return $out; }
...
try {
  ...
  if ($company_name==='') throw new Exception('Nome da empresa é obrigatório');
  $ad_city = trim($_POST['ad_city'] ?? '');
  $slug = create_slug($company_name . ($ad_city? ('-'.$ad_city):''));

  // uploads básicos
  $visitor_id = $_SESSION['visitor_db_id'] ?? null;
  if (!$visitor_id) throw new Exception('Sessão inválida (visitor)');
  $upload_base = dirname(__DIR__) . '/uploads/companies/' . $visitor_id . '/';
  ...
  if (isset($_FILES['gallery_photos']) && is_array($_FILES['gallery_photos']['name'])) {
    foreach ($_FILES['gallery_photos']['name'] as $i=>$name) {
      if ($_FILES['gallery_photos']['error'][$i]===UPLOAD_ERR_OK) {
        ...
        if (move_uploaded_file($_FILES['gallery_photos']['tmp_name'][$i], $dest)) {
          $gallery[] = '/uploads/companies/'.$visitor_id.'/'.$fn;
        }
      }
    }
  }
  ...
  // upsert por account_id (1:1)
  $existing_id = db_fetch_one("SELECT id FROM companies WHERE account_id=? LIMIT 1", [$account_id]);
  ...
  $payload = filter_for_table($pdo,'companies',$payload);

  if ($existing_id) {
    $payload['id'] = (int)$existing_id['id'];
    $set=[]; foreach($payload as $k=>$v){ if($k==='id') continue; $set[]="`$k`=:$k"; }
    if ($set) { $sql='UPDATE companies SET '.implode(',', $set).' WHERE id=:id'; $st=$pdo->prepare($sql); $st->execute($payload); }
    $id = (int)$existing_id['id'];
  } else {
    $payload['created_at'] = date('Y-m-d H:i:s');
    $cols='`'.implode('`,`', array_keys($payload)).'`'; $ph=':'.implode(',:', array_keys($payload));
    $st=$pdo->prepare("INSERT INTO companies ($cols) VALUES ($ph)"); $st->execute($payload); $id=(int)$pdo->lastInsertId();
  }

  $pdo->commit();
  echo json_encode(['status'=>'success','message'=>'Empresa guardada com sucesso.','data'=>['id'=>$id,'slug'=>$slug]]);
} catch (Throwable $e) {
  if (isset($pdo) && $pdo->inTransaction()) $pdo->rollBack();
  log_system_error('API_REGISTER_BUSINESSES_ERROR: '.$e->getMessage(),'ERROR','api_register_businesses');
  http_response_code(500); echo json_encode(['status'=>'error','message'=>'Falha ao guardar empresa.']);
}
