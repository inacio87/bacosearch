<?php
/**
 * /api/clubs.php - Endpoint para resultados de clubes
 *
 * RESPONSABILIDADES:
 * 1. Receber parâmetros de localização e filtros via POST/GET.
 * 2. Consultar a tabela `clubs` com fallback (cidade > região > país > global).
 * 3. Retornar dados em JSON para consumo por results_clubs.php.
 */
...
try {
    ...
    switch ($action) {
        case 'get_clubs':
            ...
            $results = findClubsWithFallback($location_data, $filters);
    ...
    error_log("Erro na API clubs: " . $e->getMessage() . " | Stack: " . $e->getTraceAsString());
