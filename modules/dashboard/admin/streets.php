<?php
/**
 * /modules/dashboard/admin/streets.php - Admin de Submissões de Ruas/Bares (street_posts)
 */
if (!defined('IN_BACOSEARCH')) exit('Acesso negado.');

$title = getTranslation('streets_title', $languageCode, 'admin_dashboard');
...
try {
  if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id) {
      if ($action==='approve') {
        $pdo->prepare("UPDATE street_posts SET status='active', is_active=1, updated_at=NOW() WHERE id=?")->execute([$id]);
        $success_message = getTranslation('item_approved_success', $languageCode, 'admin_dashboard');
      } elseif ($action==='reject') {
        $pdo->prepare("UPDATE street_posts SET status='rejected', is_active=0, updated_at=NOW() WHERE id=?")->execute([$id]);
        $success_message = getTranslation('item_rejected_success', $languageCode, 'admin_dashboard');
      } elseif ($action==='suspend') {
        $pdo->prepare("UPDATE street_posts SET status='suspended', is_active=0, updated_at=NOW() WHERE id=?")->execute([$id]);
        $success_message = getTranslation('item_suspended_success', $languageCode, 'admin_dashboard');
      }
    }
    header('Location: '.SITE_URL.'/admin/dashboard.php?module=streets&status='.(isset($_GET['status'])? $_GET['status'] : 'all')); exit;
  }

  $status_filter = $_GET['status'] ?? 'all';
  $cond = ''; $params=[];
  if (in_array($status_filter, ['active','pending','rejected','suspended'], true)) { $cond='WHERE s.status=:s'; $params[':s']=$status_filter; }
  $sql = "SELECT s.id, s.place_type, s.street_name, s.place_name, s.ad_city, s.ad_state, s.status, s.created_at
          FROM street_posts s $cond ORDER BY s.created_at DESC";
...
<div class="dashboard-module-wrapper">
  <div class="module-header"><h1><?= htmlspecialchars($title ?: 'Ruas'); ?></h1></div>

  <div class="time-filters">
    <a href="?module=streets&status=all" class="btn time-filter-btn <?= ($status_filter==='all')?'active':''; ?>"><?= htmlspecialchars($filter_all); ?></a>
    <a href="?module=streets&status=active" class="btn time-filter-btn <?= ($status_filter==='active')?'active':''; ?>"><?= htmlspecialchars($filter_active); ?></a>
    <a href="?module=streets&status=pending" class="btn time-filter-btn <?= ($status_filter==='pending')?'active':''; ?>"><?= htmlspecialchars($filter_pending); ?></a>
    <a href="?module=streets&status=rejected" class="btn time-filter-btn <?= ($status_filter==='rejected')?'active':''; ?>"><?= htmlspecialchars($filter_rejected); ?></a>
    <a href="?module=streets&status=suspended" class="btn time-filter-btn <?= ($status_filter==='suspended')?'active':''; ?>"><?= htmlspecialchars($filter_suspended); ?></a>
  </div>
  ...
  <div class="table-container">
    <table class="dashboard-table">
      ...
      <tbody>
        <?php if (empty($rows)): ?>
          <tr><td colspan="4"><?= htmlspecialchars(getTranslation('no_items_found', $languageCode, 'admin_dashboard') ?: 'Nenhum registro.'); ?></td></tr>
        <?php else: foreach ($rows as $r): ?>
          <tr>
            <td>
              <?php if (($r['place_type'] ?? 'street')==='bar'): ?>
                <?= htmlspecialchars($r['place_name'] ?: ''); ?>
              <?php else: ?>
                <?= htmlspecialchars($r['street_name'] ?: ''); ?>
              <?php endif; ?>
              <small style="display:block;color:#777;">(<?= htmlspecialchars($r['place_type']); ?>)</small>
            </td>
