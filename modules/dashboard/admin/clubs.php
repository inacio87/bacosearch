<?php
/**
 * /modules/dashboard/admin/clubs.php - Admin de Clubes
 */
if (!defined('IN_BACOSEARCH')) exit('Acesso negado.');

$title = getTranslation('clubs_title', $languageCode, 'admin_dashboard');
...
try {
  if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id) {
      if ($action==='approve') {
        $pdo->prepare("UPDATE clubs SET status='active', is_active=1, updated_at=NOW() WHERE id=?")->execute([$id]);
        $success_message = getTranslation('item_approved_success', $languageCode, 'admin_dashboard');
      } elseif ($action==='reject') {
        $pdo->prepare("UPDATE clubs SET status='rejected', is_active=0, updated_at=NOW() WHERE id=?")->execute([$id]);
        $success_message = getTranslation('item_rejected_success', $languageCode, 'admin_dashboard');
      } elseif ($action==='suspend') {
        $pdo->prepare("UPDATE clubs SET status='suspended', is_active=0, updated_at=NOW() WHERE id=?")->execute([$id]);
        $success_message = getTranslation('item_suspended_success', $languageCode, 'admin_dashboard');
      }
    }
    header('Location: '.SITE_URL.'/admin/dashboard.php?module=clubs&status='.(isset($_GET['status'])? $_GET['status'] : 'all')); exit;
  }

  $status_filter = $_GET['status'] ?? 'all';
  $cond = ''; $params=[];
  if (in_array($status_filter, ['active','pending','rejected','suspended'], true)) { $cond='WHERE c.status=:s'; $params[':s']=$status_filter; }
  $sql = "SELECT c.id, c.club_name, c.email, c.phone_number, c.ad_city, c.ad_state, c.status, c.created_at
          FROM clubs c $cond ORDER BY c.created_at DESC";
...
<div class="dashboard-module-wrapper">
  <div class="module-header"><h1><?= htmlspecialchars($title ?: 'Clubes'); ?></h1></div>

  <div class="time-filters">
    <a href="?module=clubs&status=all" class="btn time-filter-btn <?= ($status_filter==='all')?'active':''; ?>"><?= htmlspecialchars($filter_all); ?></a>
    <a href="?module=clubs&status=active" class="btn time-filter-btn <?= ($status_filter==='active')?'active':''; ?>"><?= htmlspecialchars($filter_active); ?></a>
    <a href="?module=clubs&status=pending" class="btn time-filter-btn <?= ($status_filter==='pending')?'active':''; ?>"><?= htmlspecialchars($filter_pending); ?></a>
    <a href="?module=clubs&status=rejected" class="btn time-filter-btn <?= ($status_filter==='rejected')?'active':''; ?>"><?= htmlspecialchars($filter_rejected); ?></a>
    <a href="?module=clubs&status=suspended" class="btn time-filter-btn <?= ($status_filter==='suspended')?'active':''; ?>"><?= htmlspecialchars($filter_suspended); ?></a>
  </div>
