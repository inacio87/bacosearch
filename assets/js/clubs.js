(function(){
  ...
  async function loadClubs() {
    toggleLoading(true);
    try {
      const res = await apiPost('get_clubs', {
        location_data: locationData,
        filters
      });
      const data = res?.data || {};
      renderBreadcrumb(data.location, data.level);
      renderResults(Array.isArray(data.clubs)?data.clubs:[]);
    } catch (e) {
      console.error(e);
  ...
  function card(it){
    ...
    const href = it.slug ? ('/club/'+encodeURIComponent(it.slug)) : 'javascript:void(0)';
  ...
  function wireFilters(){
    ...
    if (price){ price.addEventListener('input', ()=>{ filters.price_max = Number(price.value)||null; }); price.addEventListener('change', ()=> loadClubs()); }
    const dist = $('#distance-range');
    if (dist){ dist.addEventListener('input', ()=>{ filters.distance = Number(dist.value)||null; }); dist.addEventListener('change', ()=> loadClubs()); }
  }

  document.addEventListener('DOMContentLoaded', ()=>{
    try { if (window.initialClubData && window.initialClubData.clubs && window.initialClubData.clubs.length) { renderResults(window.initialClubData.clubs); } } catch(e) {}
    wireFilters();
    loadClubs();
  });
})();
