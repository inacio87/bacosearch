<?php
/**
 * /pages/register_businesses.php - Cadastro de Empresas (base)
 * Fluxo: usuário validado por email -> preenche perfil -> salva pendente -> redireciona para success (analysis_pending)
 */

$page_name = 'business_registration_flow';
require_once dirname(__DIR__) . '/core/bootstrap.php';

header('Cache-Control: no-store, no-cache, must-revalidate, max-age=0');
header('Pragma: no-cache');

$languageCode = isset($_SESSION['language']) ? strtolower(str_replace('_','-', $_SESSION['language'])) : (LANGUAGE_CONFIG['default'] ?? 'en-us');
$account_id   = filter_input(INPUT_GET, 'account_id', FILTER_VALIDATE_INT);
$errors       = $_SESSION['errors_business_form'] ?? [];
unset($_SESSION['errors_business_form']);

$show_form = false;
try {
  if (empty($account_id)) {
    throw new Exception(getTranslation('error_access_denied_business_flow', $languageCode, 'ui_messages'));
  }
...
$tmap = [
  'business_form_title'        => 'business_form',
  'business_form_subtitle'     => 'business_form',
  'label_company_name'         => 'business_form',
  'label_ad_title'             => 'business_form',
  'label_description'          => 'business_form',
  'label_keywords'             => 'business_form',
  'label_phone'                => 'business_form',
  'label_email'                => 'business_form',
  'label_website'              => 'business_form',
  'label_category'             => 'business_form',
  'label_country'              => 'business_form',
  'label_state'                => 'business_form',
  'label_city'                 => 'business_form',
  'label_street'               => 'business_form',
  'label_postal_code'          => 'business_form',
  'label_main_photo'           => 'business_form',
  'label_gallery_photos'       => 'business_form',
...
$page_title = $tr['business_form_title'] ?? 'business_form_title';
...
<main>
  <div class="main-container">
    <?php if ($show_form): ?>
    <form method="post" action="<?= $e(SITE_URL . '/api/api_register_businesses.php'); ?>" enctype="multipart/form-data" id="business-form" novalidate>
      ...
      <div class="register-container">
        <div class="form-header">
          <h1><?= $e($page_title); ?></h1>
          <p><?= $e($tr['business_form_subtitle'] ?? ''); ?></p>
        </div>
      </div>
      ...
      <div class="register-container">
        <div class="form-group">
          <label><?= $e($tr['label_company_name'] ?? 'label_company_name'); ?></label>
          <input type="text" name="company_name" maxlength="200" required class="form-control">
        </div>
        <div class="form-group">
          <label><?= $e($tr['label_ad_title'] ?? 'label_ad_title'); ?></label>
          <input type="text" name="ad_title" maxlength="200" class="form-control">
        </div>
        <div class="form-group">
        ...
        <div class="form-group">
          <label><?= $e($tr['label_keywords'] ?? 'label_keywords'); ?></label>
          <input type="text" name="keywords" maxlength="255" class="form-control" placeholder="spa, hotel, academia">
        </div>
      </div>
    ...
    <?php else: ?>
      <div class="register-container"><div class="error-message show"><strong><?= $e(getTranslation('error_access_denied_business_flow', $languageCode, 'ui_messages')); ?></strong></div></div>
    <?php endif; ?>
  </div>
</main>
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  form.addEventListener('submit', function(ev){
    ...
    btn.disabled = true; btn.innerHTML = savingText;
    fetch(form.action, { method:'POST', body: new FormData(form), headers: { 'Accept':'application/json' }})
      .then(r=>r.json())
      .then(data=>{
        if (data.status === 'success') {
          window.location.href = <?= json_encode(SITE_URL . '/success.php'); ?> + '?status=analysis_pending&company_id=' + data.data.id;
        } else { alert('Erro: ' + data.message); btn.disabled=false; btn.innerHTML=originalText; }
      })
      .catch(()=>{ alert(commErr); btn.disabled=false; btn.innerHTML=originalText; });
  });
});
</script>
