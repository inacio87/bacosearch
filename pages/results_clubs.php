<?php
/**
 * /pages/results_clubs.php - VERSÃO FINAL COM ARQUITETURA CORRIGIDA E NAVEGAÇÃO HIERÁRQUICA
 *
 * RESPONSABILIDADES:
 * 1. Ponto de entrada para a página "Resultados para Clubes".
 ...
 *
 * ÚLTIMA ATUALIZAÇÃO: 06/08/2025 - Correção da persistência do idioma da sessão.
 */
...
$page_name = 'results_clubs_page';

// Define os estilos e scripts específicos para esta página.
$page_specific_styles = [
    SITE_URL . '/assets/css/search-clubs.css' // Use search-providers.css se não houver específico
];
...
$translationContext = 'results_clubs';

$translations = [];
$keys_to_translate = [
    'results_clubs_title', 'results_clubs_meta_description', 'results_clubs_last_updated',
...
$initialFilters = [
    'category' => htmlspecialchars($_GET['category'] ?? 'liberal'),
    'price_max' => htmlspecialchars($_GET['price_max'] ?? 5000),
    'distance' => htmlspecialchars($_GET['distance'] ?? 100)
];

$page_title = $translations['results_clubs_title'] ?? 'Buscar Clubes';
$meta_description = $translations['results_clubs_meta_description'] ?? 'Encontre os melhores clubes na sua área.';
...
// PASSO 3: LÓGICA DE FALLBACK DE ANÚNCIOS e CLUBES
$club_data = findClubsWithFallback($location_data, $initialFilters);
...
<main class="search-main global-content-wrapper">
    ...
    <div class="filter-group categories">
        <button data-filter="category" data-value="liberal" class="filter-btn"><?php echo htmlspecialchars($translations['filter_category_liberal'] ?? 'Clubes Liberais'); ?></button>
            ...
            <div class="modal-body">
                <div class="planet-options">
                    <div class="planet-option" onclick="selectPlanet('earth')">
                        ...
                        <p><?php echo htmlspecialchars($translations['planet_earth_desc'] ?? 'Explore clubes do nosso planeta'); ?></p>
                    </div>
