<?php
/**
 * /clubs.php - Página pública de Clube por slug ou id
 */
require_once __DIR__ . '/core/bootstrap.php';

$languageCode = $_SESSION['language'] ?? (LANGUAGE_CONFIG['default'] ?? 'en-us');
$page_name = 'club_detail_page';
...
$row = null;
if ($id) {
  $st = $pdo->prepare("SELECT * FROM clubs WHERE id=? AND status='active' AND is_active=1");
  $st->execute([$id]);
  $row = $st->fetch(PDO::FETCH_ASSOC);
} elseif ($slug) {
  $st = $pdo->prepare("SELECT * FROM clubs WHERE slug=? AND status='active' AND is_active=1");
  $st->execute([$slug]);
  $row = $st->fetch(PDO::FETCH_ASSOC);
}
...
<main>
  <div class="main-container">
    <div class="register-container">
      ...
      <?php if (!empty($row['music_styles'])): ?><p><strong>Estilos:</strong> <?= $e($row['music_styles']); ?></p><?php endif; ?>
      <?php if (!empty($row['age_restriction'])): ?><p><strong>Idade:</strong> <?= $e($row['age_restriction']); ?></p><?php endif; ?>
      <?php if (!empty($row['entry_fee'])): ?><p><strong>Entrada:</strong> <?= $e($row['entry_fee']); ?></p><?php endif; ?>
